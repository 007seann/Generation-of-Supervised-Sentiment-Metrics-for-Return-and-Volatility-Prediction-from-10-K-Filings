...
"""
from Airflow.packages.annual_report_reader import reader
from Airflow.packages.vol_reader_fun import vol_reader, vol_reader2
...
files_path = '/opt/airflow/data/nvidia_item1a_data'
folder = 'company_df'     
...
for cik in firms_ciks:
    ...
    for filename in os.listdir(folder_path):
        if filename.rsplit('.', 1)[0] == cik:
            ...
            comb.to_csv(os.path.join(files_path, filename))
            
# Concatenating dataframes
filename = "df_0001045810"
first = True
for cik in firms_ciks:
    print('Concatenating dataframes')
    filepath = f'/opt/airflow/data/nvidia_txt_data/df_{cik}.csv'
...
df_all.to_csv(files_path + filename + '.csv')
