...
with DAG(
    dag_id="sector_10q_dict_builder",
    schedule="0 0 3 1,4,7,10 *",
    ...
    # Save File Paths
    final_save_path = f'/opt/airflow/data/{level}_{type}_data'
    csv_file_path = '/opt/airflow/data/QQQ_constituents.csv'
    data_raw_folder = f'/opt/airflow/data/{level}_{type}_html'
    sec10k_10k_extracted_folder = f'/opt/airflow/data/{level}_{type}_all_txt'
    sec10k_item1a_extracted_folder = f'/opt/airflow/data/{level}_{type}_riskFactor_txt'
    ...
    def download_executor(firm_list_path, type, start_date, end_date):
        ...
        download_fillings(QQQ_cik_ticker, data_raw_folder, doc_type, headers, start_date, end_date)
    ...
    t2_download_executor = download_executor(csv_file_path, type, start_date=start_date, end_date=end_date) 
