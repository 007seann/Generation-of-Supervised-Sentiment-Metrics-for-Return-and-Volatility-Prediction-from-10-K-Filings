        ...
        for cik in self.firms_ciks:
            ...
            for filename in os.listdir(folder_path):
                if filename.rsplit('.', 1)[0] == cik:
                    ...
                    filename = f'dtm_{cik}.csv' # either all or riskFactor
                    comb.to_csv(os.path.join(self.files_path, filename))


    def concatenate_dataframes(self, level, section, save_path):

        
        first = True
        for cik in self.firms_ciks:
            filepath = os.path.join(self.files_path, f'dtm_{cik}.csv')
        ...
        if level == "firm":
            level = cik
            filename = f'dtm_{level}_{section}.csv'
            save_path = os.path.join(save_path, level)
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            df_all.to_csv(os.path.join(save_path, filename))
        else: 
            filename = f'dtm_{level}_{section}.csv'
            df_all.to_csv(os.path.join(save_path, filename))
